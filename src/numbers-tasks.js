...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const xCoords = (x2 - x1) ** 2;
  const yCoords = (y2 - y1) ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angle = Math.acos(scalar / (module1 * module2));
  return angle;
}
...
function roundToPowerOfTen(num, pow) {
  const powDozens = 10 ** pow;
  return Math.round(num / powDozens) * powDozens;
}
